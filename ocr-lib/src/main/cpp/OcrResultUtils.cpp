...
OcrResultUtils::OcrResultUtils(JNIEnv *env, OcrResult &ocrResult, jobject boxImg) {
    ...
    jclass jOcrResultClass = env->FindClass("realcool/ocrlib/OcrResult");
...
jobject OcrResultUtils::newJPoint(cv::Point &point) {
    jclass clazz = jniEnv->FindClass("realcool/ocrlib/Point");
    if (clazz == NULL) {
...
jobject OcrResultUtils::getTextBlock(TextBlock &textBlock) {
    ...
    auto jBoxScore = (jfloat) textBlock.boxScore;
    auto jAngleScore = (jfloat) textBlock.angleScore;
    auto jAngleTime = (jdouble) textBlock.angleTime;
    jstring jText = jniEnv->NewStringUTF(textBlock.text.c_str());
    jobject jCharScores = newJScoreArray(textBlock.charScores);
    auto jCrnnTime = (jdouble) textBlock.crnnTime;
    auto jBlockTime = (jdouble) textBlock.blockTime;
    jclass clazz = jniEnv->FindClass("realcool/ocrlib/TextBlock");
    if (clazz == nullptr) {
